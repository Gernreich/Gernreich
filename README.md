## LaserMadeMusic

Musical instruments cut from 3mm Baltic birch plywood on a laser, and the cut files to
build them. Millimetre-true at `1 user unit = 1mm`, so a file prints and cuts at real
size, and every part is a flat plate joined to other flat plates.

**[All of it, indexed](https://gernreich.github.io/)** · **[The channel](https://www.youtube.com/@LaserMadeMusic)**

**Instruments** — [trumpet-coiled](https://github.com/Gernreich/trumpet-coiled) ·
[trumpet-octagonal](https://github.com/Gernreich/trumpet-octagonal) ·
[trumpet-parts](https://github.com/Gernreich/trumpet-parts) ·
[torus-octagonal](https://github.com/Gernreich/torus-octagonal) ·
[kalimba](https://github.com/Gernreich/kalimba) ·
[slapstick](https://github.com/Gernreich/slapstick) ·
[bullroarer](https://github.com/Gernreich/bullroarer) ·
[buzz-disc](https://github.com/Gernreich/buzz-disc)

**Generators** — [bore-generator](https://github.com/Gernreich/bore-generator) ·
[bore-designs](https://github.com/Gernreich/bore-designs) ·
[knotwork-soundholes](https://github.com/Gernreich/knotwork-soundholes) ·
[living-hinge](https://github.com/Gernreich/living-hinge)

**Tools** — [lasermade-tools](https://github.com/Gernreich/lasermade-tools)

Every build page records the mistakes as well as the method — a build sheet that only
says what worked is the one that costs someone a sheet of ply.
